...
scores = cross_val_score(lm,X_train,y_train, scoring = 'neg_mean_squared_error', cv= 35)
...
for i in range(1,100):
    ...
    scores = cross_val_score(lml,X_train,y_train, scoring = 'neg_mean_squared_error', cv= 35)
    rmse_scores = np.sqrt(-scores)
    rmse = np.mean(rmse_scores)
    error.append(rmse)
...
regr = RandomForestRegressor(max_features=15, n_estimators=45, random_state=100)
regr.fit(normalized_train, y_train)
scores = cross_val_score(regr, normalized_train, y_train,scoring="neg_mean_squared_error", cv=38)
...
grid_search = GridSearchCV(regr,parameters,scoring='neg_mean_absolute_error',cv=8)
